...
window.onload = () => {
  document.querySelector(".session .name").value =
    window.sessionStorage["input-name"];
};
...
inputButton.onclick = () => {
  ...
  window.localStorage.setItem(
    "tasks",
    JSON.stringify([{ id: Math.random(), title: text }])
  );
...
console.log(/ing$/gi.test(myString2));
console.log(/^we/gi.test(myString2));
console.log(/lz$/gi.test(names1));
console.log(/^\d/gi.test(names1));

console.log(names1.match(/\d\w{5}(?=z)/gi));
console.log(names1.match(/\d\w{8}(?!Z)/gi));
...
for (let value of generator) {
  // generator = (undefined) because we yiled all values in functions;
  console.log(value);
}
...
/* 
  Promise Intro And Syntax
    • Promise In JavaScript Is Like Proglse In Real Life
    • Promise Is Sorething That Happen In The Future
    • Pronise Avoid Callback Fell
    • Promise Is The Object That Represent The Status Of An Asynchronous Operation And Its Resulting Value

    - Promise Status
      - Pending: Initial State
      - Fulfilled: Coepleted Successfully
      - Rejected: failed
    Story
      • Once A Proeise Has Been Called, It Start In A Pending State
      • The Created Proeise Kill Eventually End In A Resolved State Or In A Rejected State
      - Calling The Callback Functions (Passed To Then And Catch) Upon Finishing.
    • Then
      --- Takes 2 Optional Argueents (Callback For Success Or Failure)
*/
...
/*
  ...
  Finally   => Promise is Successful Or Failed Finally Do SomeThing
*/

const myPromise1 = new Promise((resolveFunc, rejectFunc) => {
  let employees = [];
  if (employees.length === 4) {
    resolveFunc(employees);
  } else {
    rejectFunc(Error("Number Of Employees is not 4"));
  }
});
  ...
  return new Promise((resolve, reject) => {
    ...
    myRequest1.onload = function () {
      if (this.readyState === 4 && this.status === 200) {
        ...
        reject(Error(`No Data Is Found - API is wrong`));
      }
    };
    myRequest1.open("GET", aplLink, true);
    myRequest1.send();
...
getData("https://api.github.com/users/elzerowebschool/repos")
  ...
  .catch((rej) => console.log(rej)); // rej == Error(No Data...)
...
Promise.all([myFirstPromise, mySecondPromise, myThirdPromise]).then(
  (resolvedValueus) => console.log(resolvedValueus),
...
//   let users = ["Osama"];
//   if (users.length > 0) {
//     return Promise.resolve('Users are Found');
//   } else {
//     return Promise.reject(Error("Users are not Found"));
//   }
// }
...
/*
  • Await Works Only Inside Asnyc functions
    - Auait hake JavaScript Hait For The Proeise Result
    - Await Is Hore Elegant Syntax Of Getting Premise Result
*/
...
async function readData() {
  console.log("Before Promise");
  try { console.log(await myPromise2) }
...
//--------------------------------------------------------------
// The Last Of The Course, Be Happy ==> Then go to the next....
//--------------------------------------------------------------
...
// // CONST // //
//
//  2 lesson FCC => 1 part McKinsey =>
// Then 1 eps (Series The Arabic) Or 1 game Chess (If you want to rest)
// ###### After course Elzero js (DONE) => Read the book js with (FCC) and (college courses);